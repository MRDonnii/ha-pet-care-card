...
class HAPetCareCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    const ids = ["water", "water_battery", "feeder_mode", "daily_amount", "feeder_error", "container_grams", "container_percent", "refill_action"]
      .map((key) => this._config[key]).concat((this._config.meals || []).flatMap((meal) => [meal.enabled, meal.status, meal.feed_action])).filter(Boolean);
  ...
  _meal(meal, index) {
    const [label, tone] = this._status(this._state(meal.status));
    ...
    return `<article class="meal ${tone} ${enabled ? "enabled" : "disabled"}" style="--delay:${index * 65}ms">
      ...
      <div class="meal-status"><span><i></i>${this._escape(label)}</span><strong>${enabled ? "Planlagt" : "Deaktiveret"}</strong></div>
      <button class="feed" data-feed="${this._escape(meal.feed_action)}" data-name="${this._escape(meal.name)}"><ha-icon icon="mdi:silverware-fork-knife"></ha-icon>Giv nu</button>
    </article>`;
  }
  _render() {
    ...
    const completed = meals.filter((meal) => this._state(meal.status) === "done").length;
    const next = meals.find((meal) => this._on(meal.enabled) && this._state(meal.status) === "pending");
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .section-title{display:flex;align-items:end;justify-content:space-between;margin:5px 2px 9px}.section-title span{color:var(--secondary-text-color);font-size:9px;font-weight:800;letter-spacing:.09em;text-transform:uppercase}.section-title strong{font-size:10px}.meals{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,170px),1fr));gap:9px}.meal{--tone:var(--pet-muted);min-width:0;padding:12px;border:1px solid color-mix(in srgb,var(--tone) 24%,var(--pet-edge));border-radius:16px;background:linear-gradient(150deg,color-mix(in srgb,var(--tone) 7%,transparent),rgba(0,0,0,.035));animation:rise .35s both;animation-delay:var(--delay)}.meal.done{--tone:var(--pet-good)}.meal.pending{--tone:var(--pet-accent)}.meal.danger{--tone:var(--pet-danger)}.meal.disabled{opacity:.66}.meal-head{display:grid;grid-template-columns:36px minmax(0,1fr) auto;align-items:center;gap:8px}.meal-icon{display:grid;place-items:center;width:36px;height:36px;border-radius:11px;background:color-mix(in srgb,var(--tone) 13%,transparent);color:var(--tone)}.meal-icon ha-icon{width:20px;height:20px;--mdc-icon-size:20px}.meal-head div>span{display:block;color:var(--tone);font-size:9px;font-weight:800}.meal h3{overflow:hidden;margin:2px 0 0;font-size:12px;text-overflow:ellipsis;white-space:nowrap}.switch{position:relative;width:31px;height:18px;padding:0;border:1px solid var(--pet-edge);border-radius:12px;background:rgba(255,255,255,.08);cursor:pointer}.switch i{position:absolute;top:3px;left:3px;width:10px;height:10px;border-radius:50%;background:var(--pet-muted);transition:.2s}.enabled .switch{background:color-mix(in srgb,var(--pet-good) 25%,transparent)}.enabled .switch i{left:16px;background:var(--pet-good);box-shadow:0 0 7px var(--pet-good)}.meal-status{display:flex;align-items:center;justify-content:space-between;gap:7px;margin:11px 0 9px;color:var(--secondary-text-color);font-size:8px}.meal-status span{display:flex;align-items:center;gap:5px;color:var(--tone);font-weight:800;text-transform:uppercase}.meal-status i{width:6px;height:6px;border-radius:50%;background:currentColor;box-shadow:0 0 7px currentColor}.feed{display:flex;align-items:center;justify-content:center;gap:6px;width:100%;min-height:34px;border:1px solid color-mix(in srgb,var(--tone) 30%,var(--pet-edge));border-radius:10px;background:color-mix(in srgb,var(--tone) 9%,transparent);color:var(--tone);font-size:9px;font-weight:800;cursor:pointer}.feed:hover{background:color-mix(in srgb,var(--tone) 16%,transparent)}.feed ha-icon{width:15px;height:15px;--mdc-icon-size:15px}
